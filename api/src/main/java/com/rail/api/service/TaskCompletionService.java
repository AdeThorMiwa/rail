...
public class TaskCompletionService {
    ...
    public void recalcMilestone(Milestone milestone) {
        List<com.rail.api.entity.Task> tasks = taskRepository.findByMilestone(milestone);
        ...
        List<Milestone> ordered = milestoneRepository.findByGoalOrderByPosition(milestone.getGoal());
        for (int i = 0; i < ordered.size(); i++) {
            if (ordered.get(i).getId().equals(milestone.getId()) && i + 1 < ordered.size()) {
    ...
    public void recalcGoalCompletion(Goal goal) {
        if (goal.getType() == GoalType.TASK) {
            boolean allDone = taskRepository.findByGoal(goal)
                .stream().allMatch(t -> t.getStatus() == TaskStatus.DONE);
            if (!allDone) return;
        } else if (goal.getType() == GoalType.QUANTIFIED) {
            var gt = goalTargetRepository.findByGoal(goal).orElse(null);
            if (gt == null) return;
            var current = taskTargetRepository.sumActualValueByGoalAndTaskStatus(goal, TaskStatus.DONE);
            if (current == null || current.compareTo(gt.getTargetValue()) < 0) return;
        } else {
            return;
        }
